...
const restoreTodo=useTodoStore((stat)=>stat.restoreTodo)
  ...
  return (  
        ...
        <div className="w-[100%] my-2 flex flex-wrap justify-center">  
          ...
          <ul className="w-[100%] flex flex-wrap justify-center">  
            {todos.filter(todo => !todo.completed).map((todo, index) => (  
              <li key={index} className="w-[97%] flex justify-between items-center min-h-[30px] border-b-[1px] border-[#e4e4e7]">  
                <button onClick={()=>deleteTodo(todo.id)} className="w-[4%] min-h-[20px] text-[14px] text-[#22272f] flex justify-center items-center border border-[#e4e4e7] rounded-[4px]">  
                  <GoTrash />  
                </button>  
                <div className="w-[85%] flex justify-end items-center text-[14px]" onClick={()=>toggleTodo(todo.id)}>  
                  <div className="w-[85%] text-[10px] flex justify-end text-[#626c7b] font-semibold">{todo.text}</div>  
          ...
          <div className="w-[97%] min-h-[30px] bg-[#f3f4f6] flex justify-between items-center px-4">  
            <button onClick={()=>restoreTodo} className="w-[3%] h-[50%] flex justify-center items-center text-[11px] font-semibold bg-[#94a3b8] rounded-[4px]">  
              <TfiBackRight />  
            </button>  
    ...
    todos.filter(todo => todo.completed).map((todo, index) => (  
      <li key={index} className="w-[97%] flex justify-between items-center min-h-[30px] ">  
        <div className="w-[100%] flex justify-end items-center text-[14px]" onClick={() => toggleTodo(todo.id)}>  
          <div className="w-[85%] text-[10px] flex justify-end text-[#626c7b] font-semibold">{todo.text} </div>  
          <div className="w-[6%] text-[14px] flex justify-center text-[#818a98]">  
            <RiCheckboxBlankCircleLine />  
          </div>  
